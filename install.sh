...
#!/bin/bash
ffmpeg -i rtsp://$USERNAME_CAM:$PASSWORD@$IP_ADDRESS:554/stream2 -c:v copy -c:a copy -fflags nobuffer -flags low_delay -strict experimental -rtsp_transport tcp -f hls -hls_time 0.5 -hls_list_size 3 -hls_flags delete_segments+append_list+omit_endlist /var/www/html/hls/cam$CAM_NUM/stream.m3u8
EOF
